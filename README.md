# HvAOS (Human vs AI OS) — 人机协作与意图对齐操作系统
...
2. **装载规则芯片 (.mdc)**：
   - 将 `agent-rules/` 子目录下对应的 5 个规则卡片（如 `01-intent.mdc`）拷贝到项目根目录下的 `.agent/rules/` 目录中，并用人类的回答替换卡片中的占位符。
   - **如果项目涉及代码开发或特定 IDE 环境**，在终端中执行以下符号链接指令，为 Cursor 等 IDE 激活规则卡片：
     ```bash
     mkdir -p .cursor && ln -sf ../.agent/rules .cursor/rules
     ```

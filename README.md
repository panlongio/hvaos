...
## ✅ 支持矩阵（最后验证：2026-05-24）

| 环境 | 状态 | 备注 |
| :--- | :--- | :--- |
| 模板仓库自检 | 通过 | `bash scripts/verify-hvaos.sh --allow-placeholders .` |
| 本地安装回归 (coding/media/life) | 通过 | `bash scripts/eval-hvaos.sh`，当前 `3/3` |
| CI (GitHub Actions) | 已接入 | PR/Push 自动执行 verify + eval |
...
*   **使命 (Mission)**：为人类开发者和 Autonomous Agent 之间确立一套通用、轻量、高拦截率的行为边界与安全网关，帮助开发者从“改代码比自己写还累”的重改地狱中解放出来。
*   **愿景 (Vision)**：成为全平台（AI IDE、命令行 CLI Agent、多智能体框架）的底层人机对齐行为规范，并在全球开源社区的支持下共建行业级 **「规则灵魂库 (Soul Hub)」**，一键加载 React、Payment、DevOps 等场景的最佳安全防线。
*   **个人数字资产 (Digital Twin Asset)**：HvAOS 随着项目的迭代会自动吸收并记忆开发者的编码习惯、API 规约与避坑经历（自演进机制）。随着时间推移，这 5 份规则文档将生长为最贴合你个人灵魂的**「数字分身/克隆灵魂」**。在新项目一键复用该配置，AI 助手将快速继承你的全部经验品位！
...
| **`04-context.md`**<br>(上下文层) | **系统架构设计 & 历史技术债** | 每次新开对话都要重新把 README、报错日志、历史避坑经验在对话框中给 AI 解释一遍。 | **持久化记忆与已知债务灵魂**。AI 交付任务时在后台自动更新和修剪环境技术债（防膨胀限制在 5 条）。 |
...
| :--- | :--- | :--- | :--- |
| **规则生命周期** | **收敛为 5 张卡片（设计目标）**<br>存量修剪与覆写，零规则冗余 | **文件数量无限膨胀**<br>技能越积越多，形成文档垃圾堆 | **单文件杂乱肥大**<br>难以模块化，容易过时 |
| **语义冲突与幻觉** | **极低**<br>5层图层相互隔离，MDC精准挂载拦截 | **高**<br>多规则重叠造成语义冲突，引发幻觉 | **中到高**<br>大模型注意力涣散，直接选择性忽视 |
| **维护成本** | **AI 交付时自动演进 + 剪枝**<br>免去人肉维护痛苦，低维护门槛 | **无需人类维护，但内容不可控**<br>AI 总结的步骤容易出错过时 | **完全依赖人类手动重构**<br>一旦项目重构，规则迅速失效 |
| **大模型适配度** | **深度剪枝与 Token 优化**<br>可适配 Qwen、Kimi、MiniMax 等 | **限制较多**<br>通常强依赖特定的开源微调模型 | **通用，但极易被淹没**<br>长对话历史拉长后，规则直接失效 |
...
4. **验收**：初始化完成后运行 `bash scripts/verify-hvaos.sh`，确保 5 层卡片与 `.hvaos` 灵魂配置完整、且无未替换占位符。
...
- 发布时请更新 `CHANGELOG.md`，并明确写入本次评测得分与测试日期。
...
2. **挂载规则灵魂 (.mdc)**：
    > [!NOTE]
    > **「规则灵魂」概念澄清**：这里的“灵魂”为**纯软件层面的配置规则文件**（如 `.mdc` 等），**无需任何物理硬件支持**。除了支持 IDE 的被动拦截，本协议亦完全兼容命令行终端（如 Claude Code, Aider）及自定义 Agent 框架。

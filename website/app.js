...
2. **挂载规则灵魂 (.mdc)**：
   > [!NOTE]
   > **「规则灵魂」概念澄清说明**：这里的“灵魂”为**纯软件层面的配置规则文件**（如 \`.mdc\` 等），**无需任何物理硬件支持**。除了支持 IDE 的被动拦截，本协议亦完全兼容命令行终端（如 Claude Code, Aider）及自定义 Agent 框架。
...
通过把 PRD、开发规约、SOP工作流、架构债 and DoD 验收逻辑凝练收敛进 5 个高内聚的文件，并利用 MDC 拦截灵魂和自维护自演进协议，HvAOS 帮助开发者在生产环境下实现 **高一致性意图对齐**，摆脱**规则负债**与**注意力稀释**，零门槛迈入高确定性的人机协作时代并沉淀专属的数字资产。
`;

...
PRESET_DIR="$PROJECT_ROOT/presets/$PRESET_KEY"

if [ ! -d "$PRESET_DIR" ]; then
    echo -e "${RED}[ERROR] 预设目录不存在: $PRESET_DIR${NC}"
    exit 1
fi

for file in "${FILES[@]}"; do
    echo -e "📥 正在加载本地 ${CYAN}${file}.md${NC} 模板并填入个性化参数..."

    TEMP_FILE=$(mktemp)
    cp "$PRESET_DIR/${file}.md" "$TEMP_FILE"
    ...
    # Build matching MDC chips from resolved markdown
...
# Generate README-HvAos.md instruction file from local source
echo -e "📥 正在加载本地 ${CYAN}README-HvAos.md${NC} 使用说明书..."
cp "$PROJECT_ROOT/README-HvAos.md" "README-HvAos.md"
